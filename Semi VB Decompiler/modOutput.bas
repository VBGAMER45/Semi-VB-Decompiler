...
End If
    ...
    frmMain.txtCode.Text = strBuffer
End Sub
Public Function GetApiDeclareBlock(Optional ByVal scope As String = "Private") As String
'*****************************
'Build the "<scope> Declare Function ..." block for the declared (DllFunctionCall)
'APIs the project uses.  gApiList is filled in OpenVBExe from the VB external
'table: strFunctionName holds the full signature looked up in data\winapi.dat, or
'just the function name when winapi.dat has no entry (then strLibraryName is set).
'scope is "Public" when emitted in a standard module (project-wide, matching the
'source) or "Private" when emitted in a form (forms cannot host Public Declares).
'*****************************
    Dim i As Long, def As String, s As String
    On Error Resume Next
    For i = 0 To UBound(gApiList) - 1
        def = Trim$(gApiList(i).strFunctionName)
        If Len(def) > 0 Then
            If Left$(def, 7) = "Declare" Then
                s = s & scope & " " & def & vbCrLf
            Else
                'No winapi.dat signature - emit a best-effort declaration so the
                'project still names the import (params/return are not in the EXE).
                s = s & scope & " Declare Function " & def & " Lib " & cQuote & gApiList(i).strLibraryName & cQuote & " () As Long" & vbCrLf
            End If
        End If
    Next
...
Public Function GetFieldDeclBlock(ByVal owner As String) As String
    ...
    'followed by the synthesized "Private field_<off> As <type>" block for a class's
    'private backing fields (the variables its Get/Let/Set properties read and write,
    'whose names are stripped - see GetPrivFieldDeclBlock).
    Dim pubDecl As String
    On Error Resume Next
    pubDecl = gFieldDecl(owner)        'a missing key errors here - keep it from aborting
    On Error GoTo 0                     'the whole line (which would skip the private block)
    GetFieldDeclBlock = pubDecl & GetPrivFieldDeclBlock(owner)
End Function

Public Function GetPrivFieldDeclBlock(ByVal owner As String) As String
    'Synthesize "Private field_<off> As <type>" declarations for a class's private
    'instance fields, in offset order.  The field TYPES were inferred during decompile
    'from how each field is stored (gClassFieldType); a class's private backing-variable
    'descriptors are null-stripped from the typeinfo, so the NAME stays field_<off> (the
    'honest offset) - e.g. clsPacket gains `Private field_34 As Integer` /
    '`Private field_3C As String` for its ID and Name property backing variables.
    Dim s As String, i As Long, k As String, parts() As String, off As Long
    Dim offs() As Long, typs() As String, nF As Long, j As Long, tOff As Long, tTyp As String
    On Error Resume Next
    If gClassFieldType Is Nothing Then Exit Function
    nF = 0: ReDim offs(gClassFieldType.Count): ReDim typs(gClassFieldType.Count)
    'gClassFieldType is keyed "<owner>:<decimalOffset>"; the Collection cannot enumerate
    'keys, so re-derive each entry by probing plausible offsets for this owner.
    For off = 1 To &H800                        'class instance fields sit at small offsets
        k = ""
        k = gClassFieldType(owner & ":" & off)
        If Len(k) > 0 Then
            offs(nF) = off: typs(nF) = k: nF = nF + 1
        End If
    Next off
    If nF = 0 Then Exit Function
    For i = 0 To nF - 1                          'offsets are already ascending (probe order)
        s = s & "Private field_" & Hex$(offs(i)) & " As " & typs(i) & vbCrLf
    Next i
    GetPrivFieldDeclBlock = s
End Function

Public Function GetModuleGlobalDeclBlock(ByVal moduleName As String) As String
    'Synthetic Public declarations for the standard-module globals this program
    'references, assigned to their owning module by the aModulePublic address
    'range.  Module variable NAMES are stripped in native compilation, so each is
    'named global_XXXXXXXX; types are best-effort: a recovered class, an array
    'marker (the global was ReDim'd), else Long.
    On Error GoTo done
    If gNativeUsedGlobal Is Nothing Then Exit Function
    Dim base As Long, nextBase As Long
    base = ModulePublicBase(moduleName)
    If base = 0 Then Exit Function
    nextBase = NextModulePublicBase(base)
    Dim addrs() As Long, n As Long, v As Variant, va As Long
    ReDim addrs(gNativeUsedGlobal.Count + 1)
    n = 0
    For Each v In gNativeUsedGlobal
        va = CLng(v)
        If va >= base And (nextBase = 0 Or va < nextBase) Then addrs(n) = va: n = n + 1
    Next
    If n = 0 Then Exit Function
    Dim i As Long, j As Long, t As Long
    For i = 0 To n - 2
        For j = i + 1 To n - 1
            If addrs(j) < addrs(i) Then t = addrs(i): addrs(i) = addrs(j): addrs(j) = t
        Next
    Next
    Dim s As String, nm As String, typ As String
    For i = 0 To n - 1
        nm = "global_" & Right$("00000000" & Hex$(addrs(i)), 8)
        typ = "Long"
        If KeyExistsCol(gNativeGlobalClass, "g" & addrs(i)) Then typ = gNativeGlobalClass("g" & addrs(i))
        If KeyExistsCol(gNativeArrayGlobal, "g" & addrs(i)) Then
            s = s & "Public " & nm & "() As " & typ & vbCrLf
        Else
            s = s & "Public " & nm & " As " & typ & vbCrLf
        End If
    Next
    GetModuleGlobalDeclBlock = s
done:
End Function

Private Function ModulePublicBase(ByVal moduleName As String) As Long
    Dim oi As Long
    On Error Resume Next
    For oi = 0 To UBound(gObjectNameArray)
        If gObjectNameArray(oi) = moduleName Then ModulePublicBase = gObject(oi).aModulePublic: Exit Function
    Next
End Function

Private Function NextModulePublicBase(ByVal base As Long) As Long
    'Smallest aModulePublic strictly greater than base = the end of base's range.
    Dim oi As Long, b As Long, best As Long
    On Error Resume Next
    For oi = 0 To UBound(gObjectNameArray)
        b = gObject(oi).aModulePublic
        If b > base Then If best = 0 Or b < best Then best = b
    Next
    NextModulePublicBase = best
End Function

Private Function KeyExistsCol(ByRef c As Collection, ByVal key As String) As Boolean
    Dim v As Variant
    On Error Resume Next
    If c Is Nothing Then Exit Function
    v = c(key)
    KeyExistsCol = (Err.Number = 0)
End Function
...
'open project source file (F), matching what the Code tab shows.
'  - Native: the cached native decompile (modNative.GetNativeObjectCode), i.e.
'    the exact code displayed on the Code tab.
'  - P-Code with decompilation enabled: real bodies decompiled from the proc
'    table (plus EventProcList events when includeEvents is set - forms / user
'    controls).
'  - P-Code with decompilation disabled: signature-only Sub stubs.
'Used by every code-bearing exporter so forms, modules, classes and user
'controls all get their code, for both native and P-Code projects.
'*****************************
    Dim nApi As Long
    ...
    'P-Code with decompilation enabled: emit the real decompiled bodies.
    If gPcodeDecompile = True Then
        Dim k As Long, g As Long, FileNum As Integer
        Dim ProcAddr() As Long, sTemp() As String, strData As String

        frmMain.FrameStatus.Visible = True
        frmMain.txtStatus.Visible = True
        frmMain.cmdSkipProcedure.Visible = True

        'Event handlers (forms / user controls) come from EventProcList.
        If includeEvents = True Then
            For k = 0 To UBound(EventProcList) - 1
                If EventProcList(k) <> 0 Then
                    frmMain.txtStatus.Text = frmMain.txtStatus.Text & "Generating Event: " & EventProcList(k) & vbCrLf
                    strData = modPCode.DecompileProcToVB(EventProcList(k), True)
                    sTemp = Split(strData, ".")
                    If sTemp(0) = ObjectName Then
        ...
        'Ordinary Sub/Function procedures from the object's proc table.
        FileNum = FreeFile
                ...
                For g = 0 To UBound(ProcAddr)
                    If ProcAddr(g) <> 0 And ProcAddr(g) <> -1 Then
                        If ProcAddr(g) < UBound(SubName) And ProcAddr(g) > LBound(SubName) Then
                            ...
                            If UCase$(ObjectName) = UCase$(gObjectNameArray(k)) Then
                                frmMain.txtStatus.Text = frmMain.txtStatus.Text & "Generating Procedure: " & ProcAddr(g) & vbCrLf
    ...
    'P-Code, decompilation turned off: write signature stubs only.
    For nApi = 0 To UBound(gProcedureList)
        If UCase$(gProcedureList(nApi).strParent) = UCase$(ObjectName) Then
...
On Error GoTo nofile:
       ...
       F = FreeFile
        Open FilePath For Output As #F
            ...
            'Reconstruct the Win32 API Declare block (DllFunctionCall imports) only
            'when there is no standard module to host it (forms require Private
            'Declares); otherwise it is emitted once, Public, in the first module.
            If Not gHasStandardModule And Not gApiDeclEmitted Then
                Dim sApiDecl As String
                sApiDecl = GetApiDeclareBlock("Private")
            ...
            If VBVersion <> 4 Then
                ...
                Call WriteObjectProcedures(F, FormName, True)
            End If
...
'*****************************
    ...
    Open Filename For Output As #F
        ...
        Print #F, "Option Explicit"
         Print #F, "'Generated by Semi VB Decompiler - VisualBasicZone.com"
        'The project-wide Win32 API Declare block (DllFunctionCall imports) goes in
        'the FIRST standard module, once, as Public (the EXE does not record which
        'module owned each Declare; Public there is project-wide, matching source).
        If Not gApiDeclEmitted Then
            Dim sApiDecl As String
            sApiDecl = GetApiDeclareBlock("Public")
            If Len(sApiDecl) > 0 Then Print #F, sApiDecl;
            gApiDeclEmitted = True
        End If
        ...
        sGlobals = GetModuleGlobalDeclBlock(ObjectName)
        If Len(sGlobals) > 0 Then Print #F, sGlobals;
        'Decompiled procedure bodies - same as the Code tab (native cache or
        'P-Code decompile).
        Call WriteObjectProcedures(F, ObjectName, False)
...
'*****************************
    ...
    Open Filename For Output As #F
        ...
        Print #F, "Option Explicit"
        Print #F, "'Generated by Semi VB Decompiler - VisualBasicZone.com"
        'This class's instance-variable declarations: any public ones from the typeinfo
        'VarDesc, plus the synthesized `Private field_<off> As <type>` block for the
        'private backing fields its Get/Let/Set properties read and write (names are
        'stripped, so field_<off>).
        Dim sClsDecl As String
        sClsDecl = GetFieldDeclBlock(ObjectName)
        If Len(sClsDecl) > 0 Then Print #F, sClsDecl;
        'Decompiled procedure bodies - same as the Code tab (native cache or
        'P-Code decompile); previously classes only got empty stubs.
        Call WriteObjectProcedures(F, ObjectName, False)
...
'*****************************
    ...
    Open Filename For Output As #F
        ...
        Print #F, "'Generated by Semi VB Decompiler - VisualBasicZone.com"
        'Decompiled procedure bodies - same as the Code tab (native cache or
        'P-Code decompile), including control events.
        Call WriteObjectProcedures(F, ObjectName, True)
    Close #F
End Sub
