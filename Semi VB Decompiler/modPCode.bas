...
Option Explicit
'PCode Options
Public PCODEDisplayAddress As Boolean
Public PCodeDisplayHexInfo As Boolean
...
Public DataStack() As StackType

'DataTypes Constants
Public Const S_NONE As Long = 0
Public Const S_BYTE As Long = 1
Public Const S_INTEGER As Long = 2
Public Const S_LONG As Long = 3
Public Const S_STRING As Long = 4
Public Const S_CURRENCY As Long = 5
Public Const S_SINGLE As Long = 6
Public Const S_DOUBLE As Long = 7



Global Const std = 1
Global Const idx = 2
Global Const fClearMemory = 3
Global Const fOneString = 4
Global Const fReturnOneVar = 5
Global Const AddArgToStack = 6
...
'*****************************
    ...
    Open App.Path & "\dump\" & SFile & "\PcodeToVB.txt" For Output As #F
     f2 = FreeFile
    Open App.Path & "\dump\" & SFile & "\PcodeOut.txt" For Output As #f2
        ...
        Do
            ...
            For a = 0 To ProcCnt - 1
                ...
                If ProcList(a) <> 0 Then
                    ...
                    frmMain.txtStatus.Text = frmMain.txtStatus.Text & "Decoding procedure: " & ProcList(a) & "(" & a & "\" & ProcCnt & ")" & vbCrLf
                    frmMain.txtStatus.Refresh
...
'*****************************
    Dim a As Long, l As Long, t() As Byte
    Dim addrtab As Long, libname As String, fname As String, i As Long, func As Long
    Dim F As Long
    F = FreeFile
...
End Sub
Function DecompileProc(addr As Long) As String
'*****************************
'Purpose: Decompile a P-Code procedure from a CodeInfo address
'*****************************
    Dim i&, t&, t2&, u$, m As OpcodeType, a&
   ...
   On Error Resume Next
    ...
    output = Hex$(pc) + " " + FastName("proc", addr) + ":" + vbCrLf
...
'Arguement Type
    ...
    For a = 1 To Len(Mnem)
        ...
        Else
            ...
            Select Case c
                ...
                Case "x" 'External Api
                    t = File32(File16(i) * 4 + pool)
                    u = u + ReturnApiCall(t) 'MakeAddrToVB(t)
                    i = i + 2
                ...
                Case "s"
                   ...
                   ' MsgBox FileW(t)
                    Call AddTOPCodeStringList(FileW(t), t)
                    Call AddToDataStack(FileW(t), S_STRING)
                    i = i + 2
                Case "l"
                    t = ProcPC + File16(i)
                    u = u + FastName("loc", t)
                ...
                Case "1", "2", "4"
                    ...
                    Next
                    If c = 1 Then
                        Call AddToDataStack(Str(file(i)), S_BYTE)
                    ElseIf c = 2 Then
                        Call AddToDataStack(Str(File16(i)), S_INTEGER)
                    ElseIf c = 4 Then
                        Call AddToDataStack(Str(File32(i)), S_LONG)
                    End If
                ...
                Case "v" 'VTable
                   ...
                   ' u = u + MakeAddr(t)
                    ...
                    t = File32(File16(i) * 4 + pool)
                    'u = u + MakeAddr(t)
                    u = u + GetProperty(FileGUID(t), lParam) & " VTable=" & lParam & " Addr=" & t
                    'MsgBox t
                    'MsgBox FileGUID(t)
                    'Debug.Print FileGUID(t)
                    
                    i = i + 2
                
                Case "}" 'End of Procedure
                    bEndOfProcedure = True
                Case Else
    ...
    ConvertStr = u
End Function
Private Function MakeArg(ByVal t As Long) As String
    If t < 0 Then
        MakeArg = "var_" + Hex$(-t)
    Else
        MakeArg = "arg_" + Hex$(t)
...
Private Sub Init0()
    ...
    MakeOpcode 0, &H1, 1, 0, "InvalidExcode"
    MakeOpcode 0, &H2, 2, 0, "LargeBos"
    MakeOpcode 0, &H3, 1, 0, "InvalidExcode"
    MakeOpcode 0, &H4, 3, fReturnOneVar, "FLdRfVar %a"
    MakeOpcode 0, &H5, 3, std, "ImpAdLdRf %c"
    MakeOpcode 0, &H6, 3, 0, "MemLdRfVar"
    MakeOpcode 0, &H7, 5, 0, "FMemLdRf"
    'MakeOpcode 0, &H8, 3, std, "FLdPr= SR,%a"
    MakeOpcode 0, &H8, 3, std, "FLdPr= SR,%a"
    MakeOpcode 0, &H9, 5, std, "ImpAdCallHresult ::call %c(%a)"
    'MakeOpcode 0, &HA, 5, std, "ImpAdCallFPR4 ::call %c(%a)"
    MakeOpcode 0, &HA, 5, std, "ImpAdCallFPR4 %x"
    ...
    MakeOpcode 0, &HD, 5, std, "VCallHresult %v"
    MakeOpcode 0, &HE, 3, 0, "VCallFPR8"
    MakeOpcode 0, &HF, 3, std, "VCallAd"
    MakeOpcode 0, &H10, 5, std, "ThisVCallHresult %2 %c"
    MakeOpcode 0, &H11, 3, 0, "ThisVCall"
    MakeOpcode 0, &H12, 3, 0, "ThisVCallAd"
    MakeOpcode 0, &H13, 1, std, "ExitProcHresult %}"
    MakeOpcode 0, &H14, 1, std, "ExitProc %}"
    MakeOpcode 0, &H15, 1, none, "ExitProcI2"
    MakeOpcode 0, &H16, 1, none, "ExitProcR4"
    MakeOpcode 0, &H17, 1, none, "ExitProcR8"
    MakeOpcode 0, &H18, 1, none, "ExitProcCy"
    MakeOpcode 0, &H19, 3, 0, "FStAdFunc"
    MakeOpcode 0, &H1A, 3, 0, "FFree1Ad"
    MakeOpcode 0, &H1B, 3, std, "LitStr %s"
    MakeOpcode 0, &H1C, 3, std, "BranchF %l"
    MakeOpcode 0, &H1D, 3, std, "BranchT %l"
    MakeOpcode 0, &H1E, 3, std, "Branch %l"
    MakeOpcode 0, &H1F, 3, std, "CRec2Ansi %2"
    MakeOpcode 0, &H20, 3, 0, "CRec2Uni"
    MakeOpcode 0, &H21, 1, none, "FLdPrThis" 'Load VTable address into current object pointer.
    MakeOpcode 0, &H22, 3, std, "ImpAdLdPr [%c]"
    MakeOpcode 0, &H23, 3, 0, "FStStrNoPop" 'Store address of string.
    MakeOpcode 0, &H24, 3, idx, "NewIfNullPr %t" 'Create new pointer.
    MakeOpcode 0, &H25, 1, none, "PopAdLdVar" 'Load Variant data.
    MakeOpcode 0, &H26, 3, 0, "AryDescTemp"
    MakeOpcode 0, &H27, 3, std, "LitVar_Missing" 'Define missing literal Variant.
    'MakeOpcode 0, &H28, 5, 0, "LitVarI2 %a,%2" 'Define 2-Byte literal Variant.
    MakeOpcode 0, &H28, 5, std, "LitVarI2 %a,%2" 'Define 2-Byte literal Variant.
    MakeOpcode 0, &H29, -1, fClearMemory, "FFreeAd"  'Free memory used by multiple addresses.
    MakeOpcode 0, &H2A, 1, none, "ConcatStr" 'Perform <Concatination> operation on two Strings.
    MakeOpcode 0, &H2B, 3, 0, "PopTmpLdAd2"
    MakeOpcode 0, &H2C, 5, 0, "LateIdSt"
    MakeOpcode 0, &H2D, 3, 0, "AryUnlock"
    MakeOpcode 0, &H2E, 3, 0, "AryLock"
    MakeOpcode 0, &H2F, 3, fOneString, "FFree1Str" 'Free memory used by a single string.
    MakeOpcode 0, &H30, 3, 0, "PopTmpLdAd8"
    MakeOpcode 0, &H31, 3, 0, "FStStr" 'Store string.
    MakeOpcode 0, &H32, -1, fClearMemory, "FFreeStr"  'Free memory used by multiple strings.
    MakeOpcode 0, &H33, 3, std, "LdFixedStr %s"
    MakeOpcode 0, &H34, 1, none, "CStr2Ansi" 'Convert VB string to Ansi string.
    MakeOpcode 0, &H35, 3, 0, "FFree1Var" 'Free single Variant
    MakeOpcode 0, &H36, -1, fClearMemory, "FFreeVar"  'Free memory used by multiple Variants
    MakeOpcode 0, &H37, 1, idx, "PopFPR4" 'Pop 4-Byte data off FloatingPoint stack.
    MakeOpcode 0, &H38, 3, 0, "CopyBytes"
    MakeOpcode 0, &H39, 1, none, "PopFPR8" 'Pop 8-Byte data off FloatingPoint stack.
    'MakeOpcode 0, &H3A, 5, 0, "LitVarStr"
    MakeOpcode 0, &H3A, 5, std, "LitVarStr %a %s" 'Define String Variant.
    MakeOpcode 0, &H3B, 1, none, "Ary1StStrCopy" 'Store single element of array.
    MakeOpcode 0, &H3C, 1, none, "GetLastError" 'Check for error.
    MakeOpcode 0, &H3D, 3, 0, "CastAd" 'Cast object to class.
    MakeOpcode 0, &H3E, 3, 0, "FLdZeroAd" 'Push pointer to zero onto stack.
    MakeOpcode 0, &H3F, 3, 0, "CVarCy" 'Convert to Variant data type from Currency data type.
    MakeOpcode 0, &H40, 1, none, "Ary1LdRf"
    MakeOpcode 0, &H41, 1, none, "Ary1LdPr"
    MakeOpcode 0, &H42, 1, none, "CR4Var"
    MakeOpcode 0, &H43, 3, std, "FStStrCopy %a" 'Store copy of String.
    MakeOpcode 0, &H44, 3, 0, "CVarI2" 'Convert to Variant data type from 2-Byte Integer data type.
    MakeOpcode 0, &H45, 1, none, "Error"
    MakeOpcode 0, &H46, 3, 0, "CVarStr" 'Convert to Variant data type from String data type.
    MakeOpcode 0, &H47, 3, std, "StFixedStr %l %a"
    MakeOpcode 0, &H48, 3, 0, "ILdPr"
    MakeOpcode 0, &H49, 1, none, "PopAdLd4"
    MakeOpcode 0, &H4A, 1, none, "FnLenStr" 'Perform <Len> operation on String.
    MakeOpcode 0, &H4B, 3, std, "OnErrorGoto %l" 'Sets up error handler pointer.
    MakeOpcode 0, &H4C, 1, none, "FnLBound"
    MakeOpcode 0, &H4D, 5, 0, "CVarRef" 'Convert to Variant from Reference.
    MakeOpcode 0, &H4E, 3, 0, "FStVarCopyObj" 'Store, as Variant, copy of an object.
    MakeOpcode 0, &H4F, 3, 0, "MidStr"
    MakeOpcode 0, &H50, 1, none, "CI4Str" 'Convert to 4-Byte Integer from String.
    MakeOpcode 0, &H51, 3, 0, "FLdZeroAd" 'Push pointer to zero onto stack.
    MakeOpcode 0, &H52, 1, none, "Ary1StVar"
    MakeOpcode 0, &H53, 1, none, "CBoolCy" 'Convert to Boolean from Currency.
    MakeOpcode 0, &H54, 5, 0, "FMemStStrCopy"
    MakeOpcode 0, &H55, 1, none, "CI2Var" 'Convert to 2-Byte Integer from Variant.
    MakeOpcode 0, &H56, 3, 0, "NewIfNullAd"
    MakeOpcode 0, &H57, 5, 0, "LateMemLdVar"
    MakeOpcode 0, &H58, 3, 0, "MemLdPr"
    MakeOpcode 0, &H59, 3, 0, "PopTmpLdAdStr" 'Store string address.
    MakeOpcode 0, &H5A, 1, none, "Erase"
    MakeOpcode 0, &H5B, 3, 0, "FStAdFuncNoPop" 'Store address of pointer.
    MakeOpcode 0, &H5C, 3, 0, "BranchFVar"
    MakeOpcode 0, &H5D, 1, none, "HardType" 'Cast variant as specific data type."
    'MakeOpcode 0, &H5E, 5, std, "ImpAdCallI4 :: call %c(%a)" 'Call external routine.
    MakeOpcode 0, &H5E, 5, std, "ImpAdCallI4  %x" 'Call external routine.
    MakeOpcode 0, &H5F, 5, 0, "FMemLdPr"
    MakeOpcode 0, &H60, 1, none, "CStrVarTmp" 'Convert to String data type from Variant data type.
    MakeOpcode 0, &H61, 7, 0, "LateIdLdVar"
    MakeOpcode 0, &H62, 3, 0, "IStDarg"
    MakeOpcode 0, &H63, 3, 0, "LitVar_TRUE" 'Define TRUE.
    MakeOpcode 0, &H64, 5, 0, "NextI2" 'End 2-Byte-Integer based FOR/NEXT loop.
    MakeOpcode 0, &H65, 5, 0, "NextStepI2" 'End 2-Byte-Integer based FOR/NEXT loop.
    MakeOpcode 0, &H66, 5, 0, "NextI4"
    MakeOpcode 0, &H67, 5, 0, "NextStepI4"
    MakeOpcode 0, &H68, 5, 0, "NextStepR4" 'End 4-Byte-data based FOR/NEXT loop.
    MakeOpcode 0, &H69, 5, 0, "NextStepR8" 'End 8-Byte-data based FOR/NEXT loop.
    MakeOpcode 0, &H6A, 5, 0, "NextStepCy"
    MakeOpcode 0, &H6B, 3, std, "FLdI2 [%a]" 'Load 2-Byte Integer value."
    MakeOpcode 0, &H6C, 3, std, "ILdRf[%a]" 'Load reference value.
    MakeOpcode 0, &H6D, 3, 0, "FLdR8" 'Load 8-Byte Data value.
    MakeOpcode 0, &H6E, 3, 0, "FLdFPR4" 'Load 4-Byte Data value.
    MakeOpcode 0, &H6F, 3, 0, "FLdFPR8" 'Load 8-Byte Data value.
    MakeOpcode 0, &H70, 3, std, "FStI2 [%a]" 'Store 2-Byte Integer value.
    MakeOpcode 0, &H71, 3, std, "FStR4 [%a]" 'Store 4-Byte Data value.
    MakeOpcode 0, &H72, 3, 0, "FStR8" 'Store 8-Byte Data value.
    MakeOpcode 0, &H73, 3, 0, "FStFPR4" 'Store 4-Byte Data value.
    MakeOpcode 0, &H74, 3, 0, "FStFPR8" 'Store 8-Byte Data value.
    MakeOpcode 0, &H75, 3, 0, "ImpAdLdI2" 'Load 2-Byte Integer value.
    MakeOpcode 0, &H76, 3, std, "ImpAdLdI4 [%c]" 'Load 4-Byte Integer value.
    MakeOpcode 0, &H77, 3, std, "ImpAdLdCy %c" 'Load Currency value.
    MakeOpcode 0, &H78, 3, std, "ImpAdLdFPR4 %c" 'Load 4-Byte Data value.
    MakeOpcode 0, &H79, 3, std, "ImpAdLdFPR8 %c" 'Load 8-Byte Data value.
    MakeOpcode 0, &H7A, 3, std, "ImpAdStI2 %c" 'Store 2-Byte Integer value.
    MakeOpcode 0, &H7B, 3, std, "ImpAdStR4 %c" 'Store 4-Byte Data value.
    MakeOpcode 0, &H7C, 3, std, "ImpAdStCy %c" 'Store Currency value.
    MakeOpcode 0, &H7D, 3, std, "ImpAdStFPR4 %c" 'Store 4-Byte Data value.
    MakeOpcode 0, &H7E, 3, std, "ImpAdStFPR8 %c" 'Store 8-Byte Data value.
    MakeOpcode 0, &H7F, 3, std, "ILdI2 %a" 'Load 2-Byte Integer value.
    MakeOpcode 0, &H80, 3, std, "ILdI4 %a" 'Load 4-Byte Integer value.
    MakeOpcode 0, &H81, 3, std, "ILdR8 %a" 'Load 8-Byte Data value.
    MakeOpcode 0, &H82, 3, std, "ILdFPR4 %a" 'Load 4-Byte Data value.
    MakeOpcode 0, &H83, 3, std, "ILdFPR8 %a" 'Load 8-Byte Data value.
    MakeOpcode 0, &H84, 3, std, "IStI2 %c" 'Store 2-Byte Integer value.
    MakeOpcode 0, &H85, 3, std, "IStI4 %c" 'Store 4-Byte Integer value.
    MakeOpcode 0, &H86, 3, 0, "IStR8" 'Store 8-Byte Data value.
    MakeOpcode 0, &H87, 3, 0, "IStFPR4" 'Store 4-Byte Data value.
    MakeOpcode 0, &H88, 3, 0, "IStFPR8" 'Store 8-Byte Data value.
    MakeOpcode 0, &H89, 3, idx, "MemLdI2 [%2+SR]" 'Load 2-Byte Integer value.
    MakeOpcode 0, &H8A, 3, 0, "MemLdStr" 'Load String pointer.
    MakeOpcode 0, &H8B, 3, 0, "MemLdR8" 'Load 8-Byte Data value.
    MakeOpcode 0, &H8C, 3, 0, "MemLdFPR4" 'Load 4-Byte Data value.
    MakeOpcode 0, &H8D, 3, 0, "MemLdFPR8" 'Load 8-Byte Data value.
    MakeOpcode 0, &H8E, 3, 0, "MemStI2" 'Store 2-Byte Integer value.
    MakeOpcode 0, &H8F, 3, 0, "MemStI4" 'Store 2-Byte Integer value.
    MakeOpcode 0, &H90, 3, 0, "MemStR8" 'Store 8-Byte Data value.
    MakeOpcode 0, &H91, 3, 0, "MemStFPR4" 'Store 4-Byte Data value.
    MakeOpcode 0, &H92, 3, 0, "MemStFPR8" 'Store 8-Byte Data value.
    MakeOpcode 0, &H93, 5, 0, "FMemLdI2" 'Load 2-Byte Integer value.
    MakeOpcode 0, &H94, 5, 0, "FMemLdR4" 'Load 4-Byte Data value.
    MakeOpcode 0, &H95, 5, 0, "FMemLdCy" 'Load Currency value.
    MakeOpcode 0, &H96, 5, 0, "FMemLdFPR4" 'Load 4-Byte Data value.
    MakeOpcode 0, &H97, 5, 0, "FMemLdFPR8" 'Load 8-Byte Data value.
    MakeOpcode 0, &H98, 5, std, "FMemStI2 [[%a]+%2]" 'Store 2-Byte Integer value.
    MakeOpcode 0, &H99, 5, std, "FMemStI4 [[%a]+%2]" 'Store 4-Byte Integer value.
    MakeOpcode 0, &H9A, 5, std, "FMemStR8 [[%a]+%2]" 'Store 8-Byte Data value.
    MakeOpcode 0, &H9B, 5, std, "FMemStFPR4 [[%a]+%2]" 'Store 4-Byte Data value.
    MakeOpcode 0, &H9C, 5, std, "FMemStFPR8 [[%a]+%2]" 'Store 8-Byte Data value.
    MakeOpcode 0, &H9D, 1, none, "Ary1LdI2"
    MakeOpcode 0, &H9E, 1, none, "Ary1LdI4" 'Load address of single element of array.
    ...
    MakeOpcode 0, &HA9, 1, none, "AddI2" 'Perform <Addition> operation on two 2-Byte Integers.
    MakeOpcode 0, &HAA, 1, none, "AddI4" 'Perform <Addition> operation on two 4-Byte Integers.
    MakeOpcode 0, &HAB, 1, none, "AddR8" 'Perform <Addition> operation on two 8-Byte Data types.
    MakeOpcode 0, &HAC, 1, none, "AddCy" 'Perform <Addition> operation on two Currencys.
    MakeOpcode 0, &HAD, 1, none, "SubI2" 'Perform <Subtraction> operation on two 2-Byte Integers.
    MakeOpcode 0, &HAE, 1, none, "SubI4" 'Perform <Subtraction> operation on two 4-Byte Integers.
    MakeOpcode 0, &HAF, 1, none, "SubR4" 'Perform <Subtraction> operation on two 4-Byte Data types.
    MakeOpcode 0, &HB0, 1, none, "SubCy" 'Perform <Subtraction> operation on two Currencys.
    MakeOpcode 0, &HB1, 1, none, "MulI2" 'Perform <Multiplication> operation on two 2-Byte Integers.
    MakeOpcode 0, &HB2, 1, none, "MulI4" 'Perform <Multiplication> operation on two 4-Byte Integers.
    MakeOpcode 0, &HB3, 1, none, "MulR8" 'Perform <Multiplication> operation on two 8-Byte Data types.
    MakeOpcode 0, &HB4, 1, none, "MulCy" 'Perform <Multiplication> operation on two Currencys.
    MakeOpcode 0, &HB5, 1, none, "MulCyI2" 'Perform <Multiplication> operation on a Currency and a 2-Byte Integer.
    MakeOpcode 0, &HB6, 1, none, "DivR8" 'Perform <FloatingPoint Division> operation on two 8-Byte Data types.
    MakeOpcode 0, &HB7, 1, none, "UMiI2" 'Perform <Unary Minus> on 2-Byte Unsigned Integer.
    MakeOpcode 0, &HB8, 1, none, "UMiI4"
    MakeOpcode 0, &HB9, 1, none, "UMiR8" 'Perform <Unary Minus> on 8-Byte data.
    MakeOpcode 0, &HBA, 1, none, "UMiCy" 'Perform <Unary Minus> on Currency.
    MakeOpcode 0, &HBB, 1, none, "FnAbsI2" 'Perform <Absolute> operation on 2-Byte Integer.
    MakeOpcode 0, &HBC, 1, none, "FnAbsI4" 'Perform <Absolute> operation on 4-Byte Integer.
    MakeOpcode 0, &HBD, 1, none, "FnAbsR4" 'Perform <Absolute> operation on 4-Byte Data type.
    MakeOpcode 0, &HBE, 1, none, "FnAbsCy" 'Perform <Absolute> operation on Currency.
    MakeOpcode 0, &HBF, 1, none, "IDvI2" 'Perform <Integer Division> operation on two 2-Byte Integers.
    MakeOpcode 0, &HC0, 1, none, "IDvI4" 'Perform <Division> operation on two 4-Byte Integers.
    MakeOpcode 0, &HC1, 1, none, "ModI2" 'Perform <Modulous> operation on two 2-Byte Integers.
    MakeOpcode 0, &HC2, 1, none, "ModI4" 'Perform <Modulous> operation on two 4-Byte Integers.
    MakeOpcode 0, &HC3, 1, none, "NotI4" 'Perform <Not> operation on 4-Byte Integer.
    MakeOpcode 0, &HC4, 1, none, "AndI4"
    MakeOpcode 0, &HC5, 1, none, "OrI4"
    MakeOpcode 0, &HC6, 1, none, "EqI2" 'Perform <EqualTo> operation on two 2-Byte Integers.
    MakeOpcode 0, &HC7, 1, none, "EqI4" 'Perform <EqualTo> operation on two 4-Byte Integers.
    MakeOpcode 0, &HC8, 1, none, "EqR4"
    MakeOpcode 0, &HC9, 1, none, "EqCy" 'Perform <EqualTo> operation on two Currencys.
    MakeOpcode 0, &HCA, 1, none, "EqCyR8" 'Perform <EqualTo> operation on a Currency and an 8-Byte Data type.
    MakeOpcode 0, &HCB, 1, none, "NeI2"
    MakeOpcode 0, &HCC, 1, none, "NeI4" 'Perform <NotEqualTo> operation on two 4-Byte Integers.
    MakeOpcode 0, &HCD, 1, none, "NeR8"
    MakeOpcode 0, &HCE, 1, none, "NeCy" 'Perform <NotEqualTo> operation on two Currencys.
    MakeOpcode 0, &HCF, 1, none, "NeCyR8" 'Perform <NotEqualTo> operation on a Currency and an 8-Byte Data type.
    MakeOpcode 0, &HD0, 1, none, "LtI2" 'Perform <LessThan> operation on two 2-Byte integers.
    MakeOpcode 0, &HD1, 1, none, "LtI4" 'Perform <LessThan> operation on two 4-Byte integers.
    MakeOpcode 0, &HD2, 1, none, "LtR8"
    MakeOpcode 0, &HD3, 1, none, "LtCy" 'Perform <LessThan> operation on two Currencys.
    MakeOpcode 0, &HD4, 1, none, "LtCyR8" 'Perform <LessThan> operation on a Currency and an 8-Byte Data type.
    MakeOpcode 0, &HD5, 1, none, "LeI2" 'Perform <LessThanOrEqualTo> operation on two 2-Byte Integers.
    MakeOpcode 0, &HD6, 1, none, "LeI4" 'Perform <LessThanOrEqualTo> operation on two 4-Byte Integers.
    MakeOpcode 0, &HD7, 1, none, "LeR8"
    MakeOpcode 0, &HD8, 1, none, "LeCy" 'Perform <LessThanOrEqualTo> operation on two Currencys.
    MakeOpcode 0, &HD9, 1, none, "LeCyR8" 'Perform <LessThanOrEqualTo> operation on a Currency and an 8-Byte Data type.
    MakeOpcode 0, &HDA, 1, none, "GtI2" 'Test for <GreaterThan> relationship of two 2-Byte integers.
    MakeOpcode 0, &HDB, 1, none, "GtI4" 'Perform <GreaterThan> operation on two 4-Byte Integers.
    MakeOpcode 0, &HDC, 1, none, "GtR4" 'Perform <GreaterThan> operation on two 4-Byte Data types.
    MakeOpcode 0, &HDD, 1, none, "GtCy" 'Perform <GreaterThan> operation on two Currencys.
    MakeOpcode 0, &HDE, 1, none, "GtCyR8" 'Perform <GreaterThan> operation on a Currency and an 8-Byte Data type.
    MakeOpcode 0, &HDF, 1, none, "GeI2" 'Perform <GreaterThanOrEqualTo> operation on two 2-Byte Integers.
    MakeOpcode 0, &HE0, 1, none, "GeI4" 'Perform <GreaterThanOrEqualTo> operation on two 4-Byte Integers.
    MakeOpcode 0, &HE1, 1, none, "GeR8"
    MakeOpcode 0, &HE2, 1, none, "GeCy" 'Perform <GreaterThanOrEqualTo> operation on two Currencys.
    MakeOpcode 0, &HE3, 1, none, "GeCyR8" 'Perform <GreaterThanOrEqualTo> operation on a Currency and an 8-Byte Data type.
    MakeOpcode 0, &HE4, 1, none, "CI2I4" 'Convert to 2-Byte Integer data type from 4-Byte Integer data type.
    MakeOpcode 0, &HE5, 1, none, "CI2R8" 'Convert to 2-Byte Integer data type from 8-Byte Integer data type.
    MakeOpcode 0, &HE6, 1, none, "CI2Cy" 'Convert to 2-Byte Integer from Currency."
    MakeOpcode 0, &HE7, 1, none, "CI4UI1" 'Convert to 4-Byte Integer from 1-Byte Unsigned Integer.
    MakeOpcode 0, &HE8, 1, none, "CI4R8" 'Convert to 4-Byte Integer from 8-Byte Data type.
    MakeOpcode 0, &HE9, 1, none, "CI4Cy" 'Convert to 4-Byte Integer from Currency.
    MakeOpcode 0, &HEA, 1, none, "CR4R4"
    MakeOpcode 0, &HEB, 1, none, "CR8I2" 'Convert to 8-Byte data from 2-Byte Integer.
    MakeOpcode 0, &HEC, 1, none, "CR8I4" 'Convert to 8-Byte data from 4-Byte Integer.
    MakeOpcode 0, &HED, 1, none, "CR8R8"
    MakeOpcode 0, &HEE, 1, none, "CR8Cy" 'Convert to 8-Byte data from Currency.
    MakeOpcode 0, &HEF, 1, none, "CCyI2" 'Convert to Currency from 2-Byte Integer.
    MakeOpcode 0, &HF0, 1, none, "CCyI4" 'Convert to Currency from 4-Byte Integer.
    MakeOpcode 0, &HF1, 1, none, "CCyR4" 'Convert to Currency from 4-Byte Data type.
    MakeOpcode 0, &HF2, 1, none, "CDateR8" 'Convert to Date from 8-Byte Data type.
    MakeOpcode 0, &HF3, 3, std, "LitI2 %2" 'Define 2-Byte literal Integer.
    MakeOpcode 0, &HF4, 2, std, "LitI2_Byte %1" 'Define 1-Byte literal Byte.
    MakeOpcode 0, &HF5, 5, std, "LitI4 %4" 'Define 4-Byte literal Integer.
    MakeOpcode 0, &HF6, 9, 0, "LitCy"
    MakeOpcode 0, &HF7, 5, 0, "LitCy4"
    MakeOpcode 0, &HF8, 3, 0, "LitI2FP"
    MakeOpcode 0, &HF9, 5, 0, "LitR4FP"
    MakeOpcode 0, &HFA, 9, 0, "LitDate" 'Define 8-Byte literal Data.
...
Private Sub Init1()
    MakeOpcode 1, &H0, 1, 0, "InvalidExcode"
    MakeOpcode 1, &H1, 1, none, "ImpUI1" 'Perform <Implication> operation on two 1-Byte Unsigned Integers.
    MakeOpcode 1, &H2, 1, none, "ImpI4" 'Perform <Implication> operation on two 4-Byte Integers.
    MakeOpcode 1, &H3, 1, none, "ImpI4" 'Perform <Implication> operation on two 4-Byte Integers.
    MakeOpcode 1, &H4, 1, 0, "InvalidExcode"
    MakeOpcode 1, &H5, 1, 0, "InvalidExcode"
    MakeOpcode 1, &H6, 1, 0, "InvalidExcode"
    MakeOpcode 1, &H7, 3, 0, "ImpVar" 'Perform <Implication> operation on two Variants.
    MakeOpcode 1, &H8, 1, 0, "InvalidExcode"
    MakeOpcode 1, &H9, 1, none, "EqvUI1" 'Perform <Equivalence> operation on two 1-Byte Unsigned Integers.
    MakeOpcode 1, &HA, 1, none, "EqvI4" 'Perform <Equivalence> operation on two 4-Byte Integers.
    MakeOpcode 1, &HB, 1, none, "EqvI4" 'Perform <Equivalence> operation on two 4-Byte Integers.
    MakeOpcode 1, &HC, 1, 0, "InvalidExcode"
    MakeOpcode 1, &HD, 1, 0, "InvalidExcode"
    MakeOpcode 1, &HE, 1, 0, "InvalidExcode"
    MakeOpcode 1, &HF, 3, 0, "EqvVar" 'Perform <Equivalence> operation on two Variants.
    MakeOpcode 1, &H10, 1, 0, "InvalidExcode"
    MakeOpcode 1, &H11, 1, none, "XorI4" 'Perform <Xor> operation on two 4-Byte Integers.
    MakeOpcode 1, &H12, 1, none, "XorI4" 'Perform <Xor> operation on two 4-Byte Integers.
    MakeOpcode 1, &H13, 1, none, "XorI4" 'Perform <Xor> operation on two 4-Byte Integers.
    MakeOpcode 1, &H14, 1, 0, "InvalidExcode"
    MakeOpcode 1, &H15, 1, 0, "InvalidExcode"
    MakeOpcode 1, &H16, 1, 0, "InvalidExcode"
    MakeOpcode 1, &H17, 3, 0, "XorVar" 'Perform <Xor> operation on two Variants.
    MakeOpcode 1, &H18, 1, 0, "InvalidExcode"
    MakeOpcode 1, &H19, 1, none, "OrI2" 'Perform <Or> operation on two 2-Byte Integers.
    MakeOpcode 1, &H1A, 1, none, "OrI2" 'Perform <Or> operation on two 2-Byte Integers.
    MakeOpcode 1, &H1B, 1, none, "OrI2" 'Perform <Or> operation on two 2-Byte Integers.
    MakeOpcode 1, &H1C, 1, 0, "InvalidExcode"
    MakeOpcode 1, &H1D, 1, 0, "InvalidExcode"
    MakeOpcode 1, &H1E, 1, 0, "InvalidExcode"
    MakeOpcode 1, &H1F, 3, 0, "OrVar" 'Perform <Or> operation on two Variants.
    MakeOpcode 1, &H20, 1, 0, "InvalidExcode"
    MakeOpcode 1, &H21, 1, none, "AndUI1" 'Perform <And> operation on two 1-Byte Unsigned Integers.
    MakeOpcode 1, &H22, 1, none, "AndUI1" 'Perform <And> operation on two 1-Byte Unsigned Integers.
    MakeOpcode 1, &H23, 1, none, "AndUI1" 'Perform <And> operation on two 1-Byte Unsigned Integers.
    MakeOpcode 1, &H24, 1, 0, "InvalidExcode"
    MakeOpcode 1, &H25, 1, 0, "InvalidExcode"
    MakeOpcode 1, &H26, 1, 0, "InvalidExcode"
    MakeOpcode 1, &H27, 3, 0, "AndVar" 'Perform <And> operation on two Variants.
    MakeOpcode 1, &H28, 1, 0, "InvalidExcode"
    MakeOpcode 1, &H29, 1, none, "EqI2" 'Perform <EqualTo> operation on two 2-Byte Integers.
    MakeOpcode 1, &H2A, 1, none, "EqI2" 'Perform <EqualTo> operation on two 2-Byte Integers.
    MakeOpcode 1, &H2B, 1, none, "EqI4" 'Perform <EqualTo> operation on two 4-Byte Integers.
    MakeOpcode 1, &H2C, 1, none, "EqR8" 'Perform <EqualTo> operation on two 8-Byte values.
    MakeOpcode 1, &H2D, 1, none, "EqR8" 'Perform <EqualTo> operation on two 8-Byte values.
    MakeOpcode 1, &H2E, 1, none, "EqCy" 'Perform <EqualTo> operation on two Currencys.
    MakeOpcode 1, &H2F, 3, 0, "EqVar" 'Perform <EqualTo> operation on two Variants.
    MakeOpcode 1, &H30, 1, none, "EqStr" 'Perform <EqualTo> operation on two Strings.
    MakeOpcode 1, &H31, 3, 0, "EqTextVar" 'Perform <EqualTo> operation on two Variants.
    MakeOpcode 1, &H32, 1, none, "EqTextStr" 'Perform <EqualTo> operation on two Strings.
    MakeOpcode 1, &H33, 1, none, "EqVarBool" 'Perform <EqualTo> operation on two Variants.
    MakeOpcode 1, &H34, 1, none, "EqTextVarBool" ''Perform <EqualTo> operation on two Variants.
    ...
    MakeOpcode 1, &H77, 1, 0, "InvalidExcode"
    MakeOpcode 1, &H78, 1, 0, "InvalidExcode"
    MakeOpcode 1, &H79, 1, 0, "InvalidExcode"
    MakeOpcode 1, &H7A, 1, 0, "InvalidExcode"
    MakeOpcode 1, &H7B, 1, 0, "InvalidExcode"
    MakeOpcode 1, &H7C, 1, 0, "InvalidExcode"
    ...
    MakeOpcode 1, &H83, 1, 0, "InvalidExcode"
    ...
    MakeOpcode 1, &H95, 1, 0, "InvalidExcode"
    ...
    MakeOpcode 1, &H9D, 1, 0, "InvalidExcode"
    MakeOpcode 1, &H9E, 1, none, "ModUI1"
    MakeOpcode 1, &H9F, 1, none, "ModI2"
    MakeOpcode 1, &HA0, 1, none, "ModI4"
    MakeOpcode 1, &HA1, 1, 0, "InvalidExcode"
    MakeOpcode 1, &HA2, 1, 0, "InvalidExcode"
    MakeOpcode 1, &HA3, 1, 0, "InvalidExcode"
    MakeOpcode 1, &HA4, 3, 0, "ModVar"
    MakeOpcode 1, &HA5, 1, 0, "InvalidExcode"
    MakeOpcode 1, &HA6, 1, none, "IDvUI1"
    MakeOpcode 1, &HA7, 1, none, "IDvI2"
    MakeOpcode 1, &HA8, 1, none, "IDvI4"
    MakeOpcode 1, &HA9, 1, 0, "InvalidExcode"
    MakeOpcode 1, &HAA, 1, 0, "InvalidExcode"
    MakeOpcode 1, &HAB, 1, 0, "InvalidExcode"
    MakeOpcode 1, &HAC, 3, 0, "IDvVar"
    MakeOpcode 1, &HAD, 1, 0, "Unknown"
    ...
    MakeOpcode 1, &HB5, 1, 0, "InvalidExcode"
    MakeOpcode 1, &HB6, 1, 0, "InvalidExcode"
    MakeOpcode 1, &HB7, 1, 0, "InvalidExcode"
    MakeOpcode 1, &HB8, 1, 0, "InvalidExcode"
    MakeOpcode 1, &HB9, 1, none, "DivR8"
    MakeOpcode 1, &HBA, 1, none, "DivR8"
    MakeOpcode 1, &HBB, 1, 0, "InvalidExcode"
    MakeOpcode 1, &HBC, 3, 0, "DivVar"
    MakeOpcode 1, &HBD, 1, 0, "InvalidExcode"
    MakeOpcode 1, &HBE, 1, none, "NotUI1"
    MakeOpcode 1, &HBF, 1, none, "NotI4"
    MakeOpcode 1, &HC0, 1, none, "NotI4"
    MakeOpcode 1, &HC1, 1, 0, "InvalidExcode"
    MakeOpcode 1, &HC2, 1, 0, "InvalidExcode"
    MakeOpcode 1, &HC3, 1, 0, "InvalidExcode"
    MakeOpcode 1, &HC4, 3, 0, "NotVar"
    MakeOpcode 1, &HC5, 1, 0, "InvalidExcode"
    MakeOpcode 1, &HC6, 1, 0, "InvalidExcode"
    ...
    MakeOpcode 1, &HCD, 1, 0, "InvalidExcode"
    ...
    MakeOpcode 1, &HD2, 1, none, "InvalidExcode"
    MakeOpcode 1, &HD3, 1, 0, "InvalidExcode"
    ...
    MakeOpcode 1, &HDA, 1, 0, "InvalidExcode"
    MakeOpcode 1, &HDB, 1, 0, "InvalidExcode"
    MakeOpcode 1, &HDC, 1, 0, "InvalidExcode"
    MakeOpcode 1, &HDD, 1, 0, "InvalidExcode"
    ...
    MakeOpcode 1, &HE2, 1, 0, "InvalidExcode"
    MakeOpcode 1, &HE3, 1, 0, "InvalidExcode"
    MakeOpcode 1, &HE4, 1, 0, "InvalidExcode"
    MakeOpcode 1, &HE5, 1, 0, "InvalidExcode"
    ...
    MakeOpcode 1, &HEA, 1, 0, "InvalidExcode"
    ...
    MakeOpcode 1, &HF1, 1, 0, "InvalidExcode"
    ...
    MakeOpcode 1, &HF8, 1, 0, "InvalidExcode"
    MakeOpcode 1, &HF9, 1, 0, "InvalidExcode"
...
Private Sub Init2()
    ...
    MakeOpcode 2, &H3, 1, 0, "InvalidExcode"
    ...
    MakeOpcode 2, &H9, 1, 0, "InvalidExcode"
    MakeOpcode 2, &HA, 1, none, "CCyVar"
    MakeOpcode 2, &HB, 1, none, "CCyStr"
    MakeOpcode 2, &HC, 1, 0, "InvalidExcode"
    ...
    MakeOpcode 2, &H15, 1, 0, "InvalidExcode"
    ...
    MakeOpcode 2, &H1E, 1, 0, "InvalidExcode"
    ...
    MakeOpcode 2, &H36, 1, 0, "InvalidExcode"
    MakeOpcode 2, &H37, 1, 0, "InvalidExcode"
    ...
    MakeOpcode 2, &H4A, 1, 0, "InvalidExcode"
    MakeOpcode 2, &H4B, 1, none, "FnCStrVar"
    MakeOpcode 2, &H4C, 1, 0, "InvalidExcode"
    ...
    MakeOpcode 2, &H62, 1, none, "SetLastError"
    ...
    MakeOpcode 2, &H6B, 1, 0, "InvalidExcode"
    MakeOpcode 2, &H6C, 1, 0, "InvalidExcode"
    MakeOpcode 2, &H6D, 1, none, "ExitProcHresult"
    MakeOpcode 2, &H6E, 1, 0, "InvalidExcode"
    MakeOpcode 2, &H6F, 3, 0, "CheckTypeVar"
    MakeOpcode 2, &H70, 1, 0, "InvalidExcode"
    ...
    MakeOpcode 2, &H9E, 1, 0, "InvalidExcode"
    MakeOpcode 2, &H9F, 1, 0, "InvalidExcode"
    ...
    MakeOpcode 2, &HB1, 1, none, "Ary1StStrCopy" 'Store single element of array.
    ...
    MakeOpcode 2, &HC5, 1, 0, "InvalidExcode"
    MakeOpcode 2, &HC6, 1, 0, "InvalidExcode"
    MakeOpcode 2, &HC7, 1, 0, "InvalidExcode"
    ...
    MakeOpcode 2, &HD2, 1, 0, "InvalidExcode"""
    MakeOpcode 2, &HD3, 1, none, "ExitProcStr"
    MakeOpcode 2, &HD4, 1, none, "ExitProcStr"
    MakeOpcode 2, &HD5, 1, none, "ExitProcStr"
    MakeOpcode 2, &HD6, 1, 0, "InvalidExcode"
    MakeOpcode 2, &HD7, 1, 0, "InvalidExcode"
    MakeOpcode 2, &HD8, 1, 0, "InvalidExcode"
    MakeOpcode 2, &HD9, 1, 0, "InvalidExcode"
    MakeOpcode 2, &HDA, 1, 0, "InvalidExcode"
    MakeOpcode 2, &HDB, 1, 0, "InvalidExcode"
    MakeOpcode 2, &HDC, 1, 0, "InvalidExcode"
    MakeOpcode 2, &HDD, 1, 0, "InvalidExcode"
    MakeOpcode 2, &HDE, 1, 0, "InvalidExcode"
    MakeOpcode 2, &HDF, 1, 0, "InvalidExcode"
    ...
    MakeOpcode 2, &HEE, 1, 0, "InvalidExcode"
    MakeOpcode 2, &HEF, 1, 0, "InvalidExcode"
...
Private Sub Init3()
    ...
    MakeOpcode 3, &H1E, 1, 0, "InvalidExcode"
    MakeOpcode 3, &H1F, 1, 0, "InvalidExcode"
    ...
    MakeOpcode 3, &H4E, 1, 0, "InvalidExcode"
    MakeOpcode 3, &H4F, 1, 0, "InvalidExcode"
    ...
    MakeOpcode 3, &H65, 1, 0, "InvalidExcode"
    MakeOpcode 3, &H66, 1, 0, "InvalidExcode"
    ...
    MakeOpcode 3, &H6D, 1, 0, "InvalidExcode"
    ...
    MakeOpcode 3, &H7E, 1, 0, "InvalidExcode"
    MakeOpcode 3, &H7F, 1, 0, "InvalidExcode"
    ...
    MakeOpcode 3, &H92, 1, 0, "InvalidExcode"
    ...
    MakeOpcode 3, &H97, 1, 0, "InvalidExcode"
    MakeOpcode 3, &H98, 1, 0, "InvalidExcode"
    MakeOpcode 3, &H99, 3, fClearMemory, "FFree1Var"
    MakeOpcode 3, &H9A, 3, fOneString, "FFree1Str"
    ...
    MakeOpcode 3, &HAE, 1, 0, "InvalidExcode"
    MakeOpcode 3, &HAF, 1, 0, "InvalidExcode"
    ...
    MakeOpcode 3, &HCB, 5, none, "ExitForCollObj"
    MakeOpcode 3, &HCC, 5, none, "ExitForCollObj"
    MakeOpcode 3, &HCD, 5, none, "ExitForCollObj"
    MakeOpcode 3, &HCE, 5, none, "ExitForAryVar"
    MakeOpcode 3, &HCF, 5, none, "ExitForVar"
    ...
    MakeOpcode 3, &HDE, 1, 0, "InvalidExcode"
    MakeOpcode 3, &HDF, 1, 0, "InvalidExcode"
...
Private Sub Init4()
    ...
    MakeOpcode 4, &H6, 1, 0, "InvalidExcode"
    ...
    MakeOpcode 4, &HB, 1, 0, "InvalidExcode"
    ...
    MakeOpcode 4, &H26, 1, 0, "InvalidExcode"
    ...
    MakeOpcode 4, &H2B, 1, 0, "InvalidExcode"
    ...
    MakeOpcode 4, &H30, 1, 0, "InvalidExcode"
    MakeOpcode 4, &H31, 1, 0, "InvalidExcode"
    MakeOpcode 4, &H32, 1, 0, "InvalidExcode"
    MakeOpcode 4, &H33, 1, 0, "InvalidExcode"
    MakeOpcode 4, &H34, 1, 0, "InvalidExcode"
    MakeOpcode 4, &H35, 1, 0, "InvalidExcode"
    MakeOpcode 4, &H36, 1, 0, "InvalidExcode"
    MakeOpcode 4, &H37, 1, 0, "InvalidExcode"
    MakeOpcode 4, &H38, 1, 0, "InvalidExcode"
    MakeOpcode 4, &H39, 1, 0, "InvalidExcode"
    MakeOpcode 4, &H3A, 1, 0, "InvalidExcode"
    MakeOpcode 4, &H3B, 1, 0, "InvalidExcode"
    MakeOpcode 4, &H3C, 1, 0, "InvalidExcode"
    MakeOpcode 4, &H3D, 5, 0, "LitVarUI1"
    MakeOpcode 4, &H3E, 1, 0, "InvalidExcode"
    MakeOpcode 4, &H3F, 1, 0, "InvalidExcode"
    MakeOpcode 4, &H40, 1, 0, "InvalidExcode"
    MakeOpcode 4, &H41, 1, 0, "InvalidExcode"
    MakeOpcode 4, &H42, 1, 0, "InvalidExcode"
    MakeOpcode 4, &H43, 1, 0, "InvalidExcode"
    MakeOpcode 4, &H44, 1, 0, "InvalidExcode"
    MakeOpcode 4, &H45, 1, 0, "InvalidExcode"
    MakeOpcode 4, &H46, 1, 0, "InvalidExcode"
    MakeOpcode 4, &H47, 1, 0, "InvalidExcode"
    MakeOpcode 4, &H48, 1, 0, "InvalidExcode"
    MakeOpcode 4, &H49, 1, 0, "InvalidExcode"
    MakeOpcode 4, &H4A, 1, 0, "InvalidExcode"
    MakeOpcode 4, &H4B, 1, 0, "InvalidExcode"
    MakeOpcode 4, &H4C, 1, 0, "InvalidExcode"
    MakeOpcode 4, &H4D, 1, none, "SetVarVar"
    MakeOpcode 4, &H4E, 1, none, "SetVarVarFunc"
    MakeOpcode 4, &H4F, 5, 0, "ImpAdCallHresult"
    MakeOpcode 4, &H50, 1, 0, "InvalidExcode"
    MakeOpcode 4, &H51, 1, 0, "InvalidExcode"
    MakeOpcode 4, &H52, 1, 0, "InvalidExcode"
    MakeOpcode 4, &H53, 1, 0, "InvalidExcode"
    MakeOpcode 4, &H54, 1, 0, "InvalidExcode"
    MakeOpcode 4, &H55, 1, 0, "InvalidExcode"
    MakeOpcode 4, &H56, 1, 0, "InvalidExcode"
    MakeOpcode 4, &H57, 1, 0, "InvalidExcode"
    MakeOpcode 4, &H58, 1, 0, "InvalidExcode"
    MakeOpcode 4, &H59, 1, 0, "InvalidExcode"
    MakeOpcode 4, &H5A, 1, 0, "InvalidExcode"
    MakeOpcode 4, &H5B, 1, 0, "InvalidExcode"
    MakeOpcode 4, &H5C, 1, 0, "InvalidExcode"
    MakeOpcode 4, &H5D, 3, 0, "OpenFile"
    MakeOpcode 4, &H5E, 3, 0, "LockFile"
    MakeOpcode 4, &H5F, 1, 0, "InvalidExcode"
    MakeOpcode 4, &H60, 3, 0, "EraseDestruct"
    MakeOpcode 4, &H61, 3, 0, "LdFixedStr"
    MakeOpcode 4, &H62, 5, 0, "ForUI1" 'Begin 1-Byte-Byte based FOR/NEXT loop.
    MakeOpcode 4, &H63, 5, 0, "ForI2" 'Begin 2-Byte-Integer based FOR/NEXT loop
    ...
    MakeOpcode 4, &H69, 1, 0, "InvalidExcode"
    ...
    MakeOpcode 4, &H71, 1, 0, "InvalidExcode"
    ...
    MakeOpcode 4, &H7F, 1, 0, "InvalidExcode"
    ...
    MakeOpcode 4, &H94, 1, 0, "InvalidExcode"
    ...
    MakeOpcode 4, &HAB, 1, none, "LateIdNamedCallLdVar"
    ...
    MakeOpcode 4, &HC3, 1, none, "LitVarCy"
    MakeOpcode 4, &HC4, 11, none, "LitVarR8"
    MakeOpcode 4, &HC5, 1, none, "LitVarDate"
    ...
    MakeOpcode 4, &HCA, 1, 0, "InvalidExcode"
    MakeOpcode 4, &HCB, 1, 0, "InvalidExcode"
    ...
    MakeOpcode 4, &HDE, 1, 0, "InvalidExcode"
    MakeOpcode 4, &HDF, 1, 0, "InvalidExcode"
    ...
    MakeOpcode 4, &HF3, 1, 0, "InvalidExcod"
    MakeOpcode 4, &HF4, 1, 0, "InvalidExcod"
...
Private Sub Init5()
    ...
    MakeOpcode 5, &H5, 1, 0, "InvalidExcode"
    ...
    MakeOpcode 5, &H11, 1, 0, "InvalidExcode"
    ...
    MakeOpcode 5, &H1D, 1, 0, "Bos"
    MakeOpcode 5, &H1E, 5, 0, "ImpAdCallNonVirt"
    MakeOpcode 5, &H1F, 1, 0, "InvalidExcode"
    MakeOpcode 5, &H20, 1, 0, "InvalidExcode"
    MakeOpcode 5, &H21, 1, 0, "InvalidExcode"
    MakeOpcode 5, &H22, 1, 0, "InvalidExcode"
    MakeOpcode 5, &H23, 1, 0, "InvalidExcode"
    MakeOpcode 5, &H24, 1, 0, "InvalidExcode"
    MakeOpcode 5, &H25, 1, 0, "InvalidExcode"
    MakeOpcode 5, &H26, 1, 0, "InvalidExcode"
    MakeOpcode 5, &H27, 1, 0, "InvalidExcode"
    MakeOpcode 5, &H28, 1, 0, "InvalidExcode"
    MakeOpcode 5, &H29, 1, 0, "InvalidExcode"
    ...
    MakeOpcode 5, &H2F, 5, std, "ExitProcCbHresult %}"
    ...
    MakeOpcode 5, &H3F, 1, 0, "InvalidExcode"
    MakeOpcode 5, &H40, 1, 0, "InvalidExcode"
    ...
    MakeOpcode 5, &H46, 1, 0, "InvalidExcode"
    MakeOpcode 5, &H47, 0, 0, "Unknown"
    MakeOpcode 5, &H48, 0, 0, "Unknown"
    MakeOpcode 5, &H49, 0, 0, "Unknown"
    MakeOpcode 5, &H4A, 0, 0, "Unknown"
    MakeOpcode 5, &H4B, 0, 0, "Unknown"
    MakeOpcode 5, &H4C, 0, 0, "Unknown"
    MakeOpcode 5, &H4D, 0, 0, "Unknown"
    MakeOpcode 5, &H4E, 0, 0, "Unknown"
    MakeOpcode 5, &H4F, 0, 0, "Unknown"
    MakeOpcode 5, &H50, 0, 0, "Unknown"
    MakeOpcode 5, &H51, 0, 0, "Unknown"
    MakeOpcode 5, &H52, 0, 0, "Unknown"
    MakeOpcode 5, &H53, 0, 0, "Unknown"
    MakeOpcode 5, &H54, 0, 0, "Unknown"
    MakeOpcode 5, &H55, 0, 0, "Unknown"
    MakeOpcode 5, &H56, 0, 0, "Unknown"
    MakeOpcode 5, &H57, 0, 0, "Unknown"
    MakeOpcode 5, &H58, 0, 0, "Unknown"
    MakeOpcode 5, &H59, 0, 0, "Unknown"
    MakeOpcode 5, &H5A, 0, 0, "Unknown"
    MakeOpcode 5, &H5B, 0, 0, "Unknown"
    MakeOpcode 5, &H5C, 0, 0, "Unknown"
    MakeOpcode 5, &H5D, 0, 0, "Unknown"
    MakeOpcode 5, &H5E, 0, 0, "Unknown"
    MakeOpcode 5, &H5F, 0, 0, "Unknown"
    MakeOpcode 5, &H60, 0, 0, "Unknown"
    MakeOpcode 5, &H61, 0, 0, "Unknown"
    MakeOpcode 5, &H62, 0, 0, "Unknown"
    MakeOpcode 5, &H63, 0, 0, "Unknown"
    MakeOpcode 5, &H64, 0, 0, "Unknown"
    MakeOpcode 5, &H65, 0, 0, "Unknown"
    MakeOpcode 5, &H66, 0, 0, "Unknown"
    MakeOpcode 5, &H67, 0, 0, "Unknown"
    MakeOpcode 5, &H68, 0, 0, "Unknown"
    MakeOpcode 5, &H69, 0, 0, "Unknown"
    MakeOpcode 5, &H6A, 0, 0, "Unknown"
    MakeOpcode 5, &H6B, 0, 0, "Unknown"
    MakeOpcode 5, &H6C, 0, 0, "Unknown"
    MakeOpcode 5, &H6D, 0, 0, "Unknown"
    MakeOpcode 5, &H6E, 0, 0, "Unknown"
    MakeOpcode 5, &H6F, 0, 0, "Unknown"
    MakeOpcode 5, &H70, 0, 0, "Unknown"
    MakeOpcode 5, &H71, 0, 0, "Unknown"
    MakeOpcode 5, &H72, 0, 0, "Unknown"
    MakeOpcode 5, &H73, 0, 0, "Unknown"
    MakeOpcode 5, &H74, 0, 0, "Unknown"
    MakeOpcode 5, &H75, 0, 0, "Unknown"
    MakeOpcode 5, &H76, 0, 0, "Unknown"
    MakeOpcode 5, &H77, 0, 0, "Unknown"
    MakeOpcode 5, &H78, 0, 0, "Unknown"
    MakeOpcode 5, &H79, 0, 0, "Unknown"
    MakeOpcode 5, &H7A, 0, 0, "Unknown"
    MakeOpcode 5, &H7B, 0, 0, "Unknown"
    MakeOpcode 5, &H7C, 0, 0, "Unknown"
    MakeOpcode 5, &H7D, 0, 0, "Unknown"
    MakeOpcode 5, &H7E, 0, 0, "Unknown"
    MakeOpcode 5, &H7F, 0, 0, "Unknown"
    MakeOpcode 5, &H80, 0, 0, "Unknown"
    MakeOpcode 5, &H81, 0, 0, "Unknown"
    MakeOpcode 5, &H82, 0, 0, "Unknown"
    MakeOpcode 5, &H83, 0, 0, "Unknown"
    MakeOpcode 5, &H84, 0, 0, "Unknown"
    MakeOpcode 5, &H85, 0, 0, "Unknown"
    MakeOpcode 5, &H86, 0, 0, "Unknown"
    MakeOpcode 5, &H87, 0, 0, "Unknown"
    MakeOpcode 5, &H88, 0, 0, "Unknown"
    MakeOpcode 5, &H89, 0, 0, "Unknown"
    MakeOpcode 5, &H8A, 0, 0, "Unknown"
    MakeOpcode 5, &H8B, 0, 0, "Unknown"
    MakeOpcode 5, &H8C, 0, 0, "Unknown"
    MakeOpcode 5, &H8D, 0, 0, "Unknown"
    MakeOpcode 5, &H8E, 0, 0, "Unknown"
    MakeOpcode 5, &H8F, 0, 0, "Unknown"
    MakeOpcode 5, &H90, 0, 0, "Unknown"
    MakeOpcode 5, &H91, 0, 0, "Unknown"
    MakeOpcode 5, &H92, 0, 0, "Unknown"
    MakeOpcode 5, &H93, 0, 0, "Unknown"
    MakeOpcode 5, &H94, 0, 0, "Unknown"
    MakeOpcode 5, &H95, 0, 0, "Unknown"
    MakeOpcode 5, &H96, 0, 0, "Unknown"
    MakeOpcode 5, &H97, 0, 0, "Unknown"
    MakeOpcode 5, &H98, 0, 0, "Unknown"
    MakeOpcode 5, &H99, 0, 0, "Unknown"
    MakeOpcode 5, &H9A, 0, 0, "Unknown"
    MakeOpcode 5, &H9B, 0, 0, "Unknown"
    MakeOpcode 5, &H9C, 0, 0, "Unknown"
    MakeOpcode 5, &H9D, 0, 0, "Unknown"
    MakeOpcode 5, &H9E, 0, 0, "Unknown"
    MakeOpcode 5, &H9F, 0, 0, "Unknown"
    MakeOpcode 5, &HA0, 0, 0, "Unknown"
    MakeOpcode 5, &HA1, 0, 0, "Unknown"
    MakeOpcode 5, &HA2, 0, 0, "Unknown"
    MakeOpcode 5, &HA3, 0, 0, "Unknown"
    MakeOpcode 5, &HA4, 0, 0, "Unknown"
    MakeOpcode 5, &HA5, 0, 0, "Unknown"
    MakeOpcode 5, &HA6, 0, 0, "Unknown"
    MakeOpcode 5, &HA7, 0, 0, "Unknown"
    MakeOpcode 5, &HA8, 0, 0, "Unknown"
    MakeOpcode 5, &HA9, 0, 0, "Unknown"
    MakeOpcode 5, &HAA, 0, 0, "Unknown"
    MakeOpcode 5, &HAB, 0, 0, "Unknown"
    MakeOpcode 5, &HAC, 0, 0, "Unknown"
    MakeOpcode 5, &HAD, 0, 0, "Unknown"
    MakeOpcode 5, &HAE, 0, 0, "Unknown"
    MakeOpcode 5, &HAF, 0, 0, "Unknown"
    MakeOpcode 5, &HB0, 0, 0, "Unknown"
    MakeOpcode 5, &HB1, 0, 0, "Unknown"
    MakeOpcode 5, &HB2, 0, 0, "Unknown"
    MakeOpcode 5, &HB3, 0, 0, "Unknown"
    MakeOpcode 5, &HB4, 0, 0, "Unknown"
    MakeOpcode 5, &HB5, 0, 0, "Unknown"
    MakeOpcode 5, &HB6, 0, 0, "Unknown"
    MakeOpcode 5, &HB7, 0, 0, "Unknown"
    MakeOpcode 5, &HB8, 0, 0, "Unknown"
    MakeOpcode 5, &HB9, 0, 0, "Unknown"
    MakeOpcode 5, &HBA, 0, 0, "Unknown"
    MakeOpcode 5, &HBB, 0, 0, "Unknown"
    MakeOpcode 5, &HBC, 0, 0, "Unknown"
    MakeOpcode 5, &HBD, 0, 0, "Unknown"
    MakeOpcode 5, &HBE, 0, 0, "Unknown"
    MakeOpcode 5, &HBF, 0, 0, "Unknown"
    MakeOpcode 5, &HC0, 0, 0, "Unknown"
    MakeOpcode 5, &HC1, 0, 0, "Unknown"
    MakeOpcode 5, &HC2, 0, 0, "Unknown"
    MakeOpcode 5, &HC3, 0, 0, "Unknown"
    MakeOpcode 5, &HC4, 0, 0, "Unknown"
    MakeOpcode 5, &HC5, 0, 0, "Unknown"
    MakeOpcode 5, &HC6, 0, 0, "Unknown"
    MakeOpcode 5, &HC7, 0, 0, "Unknown"
    MakeOpcode 5, &HC8, 0, 0, "Unknown"
    MakeOpcode 5, &HC9, 0, 0, "Unknown"
    MakeOpcode 5, &HCA, 0, 0, "Unknown"
    MakeOpcode 5, &HCB, 0, 0, "Unknown"
    MakeOpcode 5, &HCC, 0, 0, "Unknown"
    MakeOpcode 5, &HCD, 0, 0, "Unknown"
    MakeOpcode 5, &HCE, 0, 0, "Unknown"
    MakeOpcode 5, &HCF, 0, 0, "Unknown"
    MakeOpcode 5, &HD0, 0, 0, "Unknown"
    MakeOpcode 5, &HD1, 0, 0, "Unknown"
    MakeOpcode 5, &HD2, 0, 0, "Unknown"
    MakeOpcode 5, &HD3, 0, 0, "Unknown"
    MakeOpcode 5, &HD4, 0, 0, "Unknown"
    MakeOpcode 5, &HD5, 0, 0, "Unknown"
    MakeOpcode 5, &HD6, 0, 0, "Unknown"
    MakeOpcode 5, &HD7, 0, 0, "Unknown"
    MakeOpcode 5, &HD8, 0, 0, "Unknown"
    MakeOpcode 5, &HD9, 0, 0, "Unknown"
    MakeOpcode 5, &HDA, 0, 0, "Unknown"
    MakeOpcode 5, &HDB, 0, 0, "Unknown"
    MakeOpcode 5, &HDC, 0, 0, "Unknown"
    MakeOpcode 5, &HDD, 0, 0, "Unknown"
    MakeOpcode 5, &HDE, 0, 0, "Unknown"
    MakeOpcode 5, &HDF, 0, 0, "Unknown"
    MakeOpcode 5, &HE0, 0, 0, "Unknown"
    MakeOpcode 5, &HE1, 0, 0, "Unknown"
    MakeOpcode 5, &HE2, 0, 0, "Unknown"
    MakeOpcode 5, &HE3, 0, 0, "Unknown"
    MakeOpcode 5, &HE4, 0, 0, "Unknown"
    MakeOpcode 5, &HE5, 0, 0, "Unknown"
    MakeOpcode 5, &HE6, 0, 0, "Unknown"
    MakeOpcode 5, &HE7, 0, 0, "Unknown"
    MakeOpcode 5, &HE8, 0, 0, "Unknown"
    MakeOpcode 5, &HE9, 0, 0, "Unknown"
    MakeOpcode 5, &HEA, 0, 0, "Unknown"
    MakeOpcode 5, &HEB, 0, 0, "Unknown"
    MakeOpcode 5, &HEC, 0, 0, "Unknown"
    MakeOpcode 5, &HED, 0, 0, "Unknown"
    MakeOpcode 5, &HEE, 0, 0, "Unknown"
    MakeOpcode 5, &HEF, 0, 0, "Unknown"
    MakeOpcode 5, &HF0, 0, 0, "Unknown"
    MakeOpcode 5, &HF1, 0, 0, "Unknown"
    MakeOpcode 5, &HF2, 0, 0, "Unknown"
    MakeOpcode 5, &HF3, 0, 0, "Unknown"
    MakeOpcode 5, &HF4, 0, 0, "Unknown"
    MakeOpcode 5, &HF5, 0, 0, "Unknown"
    MakeOpcode 5, &HF6, 0, 0, "Unknown"
    MakeOpcode 5, &HF7, 0, 0, "Unknown"
    MakeOpcode 5, &HF8, 0, 0, "Unknown"
    MakeOpcode 5, &HF9, 0, 0, "Unknown"
    MakeOpcode 5, &HFA, 0, 0, "Unknown"
    MakeOpcode 5, &HFB, 0, 0, "Unknown"
    MakeOpcode 5, &HFC, 0, 0, "Unknown"
    MakeOpcode 5, &HFD, 0, 0, "Unknown"
    MakeOpcode 5, &HFE, 0, 0, "Unknown"
    MakeOpcode 5, &HFF, 0, 0, "Unknown"
...
'$=string
'%=integer
Private Sub MakeOpcode(ByVal index As Long, ByVal Num As Long, ByVal length As Long, ByVal Flag As Long, ByVal Mnem As String)
    OPCode(index, Num).Mnemonic = Mnem
    OPCode(index, Num).Flag = Flag
    OPCode(index, Num).Size = length
End Sub

Private Function MakeHex(a) As String
    If a < 16 Then MakeHex = "0" + Hex$(a) Else MakeHex = Hex$(a)
End Function


Private Function File16(addr As Long) As Long
'******************************
'Get Int16 off array
'******************************
    Dim iNum As Integer
    CopyMemory iNum, file(addr), 2
    File16 = iNum
End Function
Private Function FileGUID(ByVal lAddress As Long) As String
'******************************
'Get guid off stack
'******************************
    Dim bArray(15) As Byte
    Dim bData As Byte
    CopyMemory bData, file(lAddress), 1
    bArray(0) = bData
    CopyMemory bData, file(lAddress + 1), 1
    bArray(1) = bData
    CopyMemory bData, file(lAddress + 2), 1
    bArray(2) = bData
    CopyMemory bData, file(lAddress + 3), 1
    bArray(3) = bData
    CopyMemory bData, file(lAddress + 4), 1
    bArray(4) = bData
    CopyMemory bData, file(lAddress + 5), 1
    bArray(5) = bData
    CopyMemory bData, file(lAddress + 6), 1
    bArray(6) = bData
    CopyMemory bData, file(lAddress + 7), 1
    bArray(7) = bData
    CopyMemory bData, file(lAddress + 8), 1
    bArray(8) = bData
    CopyMemory bData, file(lAddress + 9), 1
    bArray(9) = bData
    CopyMemory bData, file(lAddress + 10), 1
    bArray(10) = bData
    CopyMemory bData, file(lAddress + 11), 1
    bArray(11) = bData
    CopyMemory bData, file(lAddress + 12), 1
    bArray(12) = bData
    CopyMemory bData, file(lAddress + 13), 1
    bArray(13) = bData
    CopyMemory bData, file(lAddress + 14), 1
    bArray(14) = bData
    CopyMemory bData, file(lAddress + 15), 1
    bArray(15) = bData
   ...
   strFinal = strFinal & "-" & strArray(10) & strArray(11) & strArray(12) & strArray(13) & strArray(14) & strArray(15) & "}"
    ...
    FileGUID = strFinal
End Function
Private Function File32(ByVal addr As Long) As Long
'******************************
'Get Int32 off file array
'******************************
On Error Resume Next
    Dim t As Long
...
Private Function FileZ(ByVal addr As Long) As String
'******************************
...
Private Function FileW(ByVal addr As Long) As String
    Dim t$, a&
    If addr > 0 Then
        a = addr
        Do While File16(a) <> 0
...
Private Sub AddMap(a As Long)
On Error Resume Next
    Map(a&) = Map(a) Or 1
End Sub

Private Sub AddRef(a As Long, src As Long)
On Error Resume Next
    Map(a&) = Map(a) Or 2
...
Private Function HasMap(a As Long) As Long
On Error Resume Next
    If Map(a) And 1 Then HasMap = -1
End Function

Private Function HasRef(a As Long) As Long
On Error Resume Next
...
Sub AddProc(ByVal addr As Long)
...
Private Function FastName(prefix As String, addr As Long) As String
...
Private Function MakeAddr(ByRef t As Long) As String
    Dim u$
...
Private Function ConvertStrToVB(Mnem As String, addr As Long, pool As Long, origpool As Long, ProcPC As Long) As String
'Arguement Type
    Dim a&, c$, t&, u$, i&, J&
    'MsgBox Mnem
    i = addr
        ...
        Else
            ...
            Select Case c
                ...
                Case "v" 'VTable
                    ...
                    i = i + 2
                    t = File32(File16(i) * 4 + pool)
                    u = u + GetProperty(FileGUID(t), lParam) & " Param=" & lParam & " Addr=" & t
...
Function MakeAddrToVB(t As Long) As String
    ...
    MakeAddrToVB = u
End Function
Private Sub AddTOPCodeStringList(ByVal strData As String, ByVal lAddr As Long)
    If ShowPCodeStringAddress = False Then
        strPCODEStringList = strPCODEStringList & strData & vbCrLf
    Else
        strPCODEStringList = strPCODEStringList & strData & "  -Address: " & lAddr & vbCrLf
    End If
End Sub
...
Private Function GetProperty(ByVal strGuid As String, ByVal lCode As Long) As String
    ...
    With cTypeInfo
        For i = 0 To .TypeInfoCount - 1
            .SelectTypeInfo i
            If .TypeInfoGUID = strGuid Then

               ' MsgBox cTypeInfo.TypeInfoName
            GetProperty = cTypeInfo.TypeInfoName & "_"
                'MsgBox strGuid
               ' MsgBox i
               .SelectTypeInfo i + 1
          ' MsgBox i & cTypeInfo.TypeInfoName
            ...
            End If
        Next i
        GetProperty = "Unknown GUID -" & strGuid
...
End Function
Private Function ReturnApiCall(ByVal lAddress As Long) As String
    
    'First Check if its a MSVBVM dll such as varptr

    Dim lAddrApiCall As Long
    lAddrApiCall = File32(lAddress + 12)
   ' MsgBox lAddress & " " & lAddrApiCall
    If lAddrApiCall > UBound(file) Then
      If UBound(DataStack) > 0 Then
        Dim d As Long
        Dim strData As String
        strData = "("
        For d = UBound(DataStack) - 1 To 0 Step -1
            If d <> 0 Then
                If DataStack(d).DataType = S_STRING Then
                    strData = strData & cQuote & DataStack(d).Data & cQuote & ", "
                ElseIf DataStack(d).DataType <> S_NONE Then
                    strData = strData & DataStack(d).Data & ", "
                End If
        ...
        ReDim DataStack(0)
        strData = strData & ")"
        ReturnApiCall = Replace(MakeAddrToVB(lAddress), "()", strData)
      Else
       ReturnApiCall = MakeAddrToVB(lAddress)
      End If
      Exit Function
    End If
    ...
    Dim lLibname As Long
    Dim lFunctionName As Long
    lLibname = File32(lAddrApiCall)
    lFunctionName = File32(lAddrApiCall + 4)
    'MsgBox "Lib A: " & lLibname
    'MsgBox "FName A: " & lFunctionName
    strLibname = FileZ(lLibname)
    strFunctionName = FileZ(lFunctionName)
    'MsgBox "STR LIB " & strLibname
    'MsgBox "STR FUnc: " & strFunctionName
    ReturnApiCall = strLibname & "." & strFunctionName
End Function
Public Sub AddToDataStack(ByVal Data As String, ByVal DataType As Long, Optional DataId As String)
    'Add value to stack
    DataStack(UBound(DataStack)).Data = Data
    DataStack(UBound(DataStack)).DataType = DataType
    DataStack(UBound(DataStack)).DataId = DataId
    'Resize the stack
    ReDim Preserve DataStack(UBound(DataStack) + 1)
End Sub
Private Sub RemoveFromStack()

End Sub
Private Sub FreeOneString()
    Dim i As Long
    For i = UBound(DataStack) To 0 Step -1
        If DataStack(i).DataType = S_STRING Then
            DataStack(i).Data = ""
            DataStack(i).DataId = ""
            DataStack(i).DataType = 0
            Exit For
        End If
    Next i
End Sub
Public Function ReturnOneVar() As String
    Dim i As Long
    For i = UBound(DataStack) To 0 Step -1
            If DataStack(i).DataType <> S_NONE Then
                If DataStack(i).DataType = S_STRING Then
                    DataStack(i).DataType = 0
                    DataStack(i).DataId = ""
                    ReturnOneVar = cQuote & DataStack(i).Data & cQuote
                    DataStack(i).Data = ""
                    Exit Function
                Else
                    DataStack(i).DataType = 0
                    DataStack(i).DataId = ""
                    ReturnOneVar = DataStack(i).Data
                    DataStack(i).Data = ""
                    Exit Function
                End If
                Exit For
