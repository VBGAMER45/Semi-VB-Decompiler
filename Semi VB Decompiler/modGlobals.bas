...
Sub PrintReadMe()
    ...
    Open App.Path & "\ReadMe.txt" For Output As #F
        ...
        Print #F, "2. Features"
        Print #F, "3. Command Line Options"
        Print #F, "4. Questions?"
        Print #F, "5. Bugs"
        Print #F, "6. Contact"
        Print #F, "7. Credits"
        ...
        Print #F, "1. What's New?"
        Print #F, ""
        Print #F, "   Version 2.0 Build 2.0.7"
        Print #F, "   .Net decompiler rebuilt.  It now reconstructs classes, fields, properties and methods to C# and VB.NET (in addition to a full IL disassembly).  The reconstructed types are browsable under a new "".Net Classes"" node in the project tree, with C#, VB.NET and IL views per class and the method list for navigation.  Method bodies are reconstructed including if/else, while/do-while, switch and try/catch/finally, falling back to labeled goto for control flow that cannot be structured.  Added File > Build .Net Solution to export a Visual Studio solution (.sln with C# and VB.NET projects, one source file per class).  Added a headless command-line mode for decompiling and generating projects/solutions without the UI - see ""3. Command Line Options"" below.  The .Net helper DLL is no longer required (its BitConverter helpers are now implemented in pure VB6)."
        Print #F, ""
        Print #F, "   Version 2.0"
        ...
        Print #F, "     Decompiling .Net assemblies to C# and VB.NET, with a full IL disassembly"
        Print #F, "     Browse reconstructed .Net classes (C# / VB.NET / IL) in the project tree"
        Print #F, "     Build a .Net solution (.sln with C# and VB.NET projects) from an assembly"
        Print #F, "     Command line / batch mode for headless decompiling and project generation"
        ...
        Print #F, "3. Command Line Options"
        Print #F, ""
        Print #F, "   Semi VB Decompiler can run without its window (headless) so you can"
        Print #F, "   decompile files and generate projects from a script or batch file."
        Print #F, ""
        Print #F, "   Usage:"
        Print #F, "     SemiVBDecompiler.exe <inputfile> [/out <dir>] [/vbp] [/dism] [/solution]"
        Print #F, ""
        Print #F, "   <inputfile>   The VB 4/5/6 or .Net exe, dll or ocx to decompile.  This is"
        Print #F, "                 the first argument that is not a switch."
        Print #F, "   /out <dir>    Directory to write the generated VB project or .Net solution"
        Print #F, "                 into.  If omitted it defaults to the file's dump folder."
        Print #F, "                 (The intermediate decompile output - IL, file report, P-Code,"
        Print #F, "                 images - is always written under the dump\<filename> folder"
        Print #F, "                 next to the program, the same as the GUI.)"
        Print #F, "   /vbp          Generate a VB project (.vbp plus all forms, modules, classes,"
        Print #F, "                 user controls, property pages, user documents and designers)."
        Print #F, "   /dism         Generate a VB project that uses the raw native disassembly for"
        Print #F, "                 each procedure instead of the decompiled VB code."
        Print #F, "   /solution     For a .Net assembly, build a Visual Studio solution containing"
        Print #F, "                 a C# project and a VB.NET project, one source file per class."
        Print #F, "   /?            Show usage."
        Print #F, ""
        Print #F, "   Notes:"
        Print #F, "   - The correct action is chosen automatically for the file type: a VB 4/5/6"
        Print #F, "     file always produces a .vbp project and a .Net assembly always produces a"
        Print #F, "     solution, regardless of which generate switch is passed."
        Print #F, "   - With no generate switch the file is only decompiled (its dump folder is"
        Print #F, "     populated); no project or solution is written."
        Print #F, "   - No message boxes or prompts appear in command line mode.  A decompile.log"
        Print #F, "     is written to the output directory, the program returns an exit code of 0"
        Print #F, "     on success or non-zero on error, and then closes automatically."
        Print #F, "   - Quote any path that contains spaces."
        Print #F, "   - Run the program from (or alongside) its install folder so it can find its"
        Print #F, "     data files (languages, API list, vb6.olb, required OCX/DLLs, etc)."
        Print #F, ""
        Print #F, "   Examples:"
        Print #F, "     SemiVBDecompiler.exe ""C:\Apps\MyApp.exe"" /out ""C:\Out"" /vbp"
        Print #F, "     SemiVBDecompiler.exe ""C:\Apps\MyApp.exe"" /out ""C:\Out"" /dism"
        Print #F, "     SemiVBDecompiler.exe ""C:\Libs\MyNet.dll"" /out ""C:\Out"" /solution"
        Print #F, "     SemiVBDecompiler.exe ""C:\Apps\MyApp.exe"""
        Print #F, ""
        Print #F, "4. Questions?"
        ...
        Print #F, "   A. Head over to https://sandsprite.com/vb-reversing/  tons of information on vb decompiling."
        Print #F, ""
        Print #F, "5. Bugs"
